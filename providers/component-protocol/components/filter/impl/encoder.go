...
// EncodeData .
func (d *DefaultFilter) EncodeData(srcStructPtr interface{}, dstRawPtr *cptype.ComponentData) {
	// custom -> struct
	if custom, ok := d.Impl.(filter.CustomData); ok {
		custom.EncodeFromCustomData(custom.CustomDataPtr(), srcStructPtr.(*filter.Data))
	}
	// struct -> raw
...
// EncodeInParams .
func (d *DefaultFilter) EncodeInParams(srcStructPtr interface{}, dstRawPtr *cptype.InParams) {
